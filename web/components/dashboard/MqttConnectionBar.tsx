...
/** 연결 버튼·상태 뱃지 — 힌트와 분리 구독으로 MQTT 수신 시 불필요 리페인트 감소 */
const MqttConnectionBarControls = memo(function MqttConnectionBarControls() {
...
const MqttConnectionHintLine = memo(function MqttConnectionHintLine() {
  const { hint } = useMqttHint();
  return hint ? (
    <p className="text-xs text-foreground/90">{hint}</p>
  ) : null;
});
