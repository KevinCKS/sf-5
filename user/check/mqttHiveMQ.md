...
| 발행 API | `POST /api/mqtt/publish` — 로그인 세션, allowlist 토픽만. 서버는 `MQTT_BROKER_URL` 등 **서버 전용** 변수로 브로커에 연결. 액추 토픽 발행 후 `actuator_controls` 이력 저장 (`web/lib/mqtt/actuatorTopics.ts`) |
